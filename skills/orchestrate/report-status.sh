...
[ -f "$HOME/.claude/pipeline/config.local.sh" ] && { _mp=$(grep -E "^MAX_PUSHES_PER_DAY=[0-9]+" "$HOME/.claude/pipeline/config.local.sh" | tail -1 | sed -E "s/^MAX_PUSHES_PER_DAY=([0-9]+).*/\1/"); [ -n "$_mp" ] && MAX_PUSHES_PER_DAY=$_mp; }
