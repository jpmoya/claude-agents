...
3. Create the feature branch and worktree off the repo's **integration branch**: `staging` where `git ls-remote --heads origin staging` returns a ref (scheduler, quoting tool), otherwise `main`. `git fetch origin <base> && git worktree add .worktrees/<branch> -b <branch> origin/<base>`. Branch name: `issue-<N>-<slug>`. Never touch the main checkout. **Revision dispatch** (a `TESTS FAIL` or `TEST DEFECT` sent you back): the branch exists — `git fetch origin <branch> && git worktree add .worktrees/<branch> <branch>`, never `-b`.
