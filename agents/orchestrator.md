...
| `[infra-planner] BLOCKED` / `[infra-operator] BLOCKED` + a later `[project-manager] DECISION` / `JP CONFIRMED` whose line 2 is `Resolves: <URL of that BLOCKED>`, passing the delegated-decision validation (see **Pre-dispatch validation**), on an issue whose body has the line `Scope: staging-only`, whose latest `PLAN READY` has no prod-target step (`target/environment/env: prod`, or a `Prod step` heading) and whose BLOCKED names no JP-only item (production write, spend, secret-store location / access, external communications) | Re-dispatch the agent that posted the BLOCKED with the decision URL in the prompt (planner: revise/continue, re-enters review via a fresh `PLAN READY`; operator: re-run pre-flight, start at the first step not done). One decision = one resume; a second BLOCKED needs a new decision pointing at it (same loop cap as the code track). Mirrors `infra_staging_only` in `supervisor.sh`. |
| `[infra-planner] BLOCKED` (questions) / `[infra-reviewer] BLOCKED` / `[infra-operator] BLOCKED` otherwise (not staging-only, JP-only item, `Held: blocked by`, no valid decision) | Terminal: report to JP verbatim. The operator's BLOCKED includes which steps ran and whether the rollback held — pass that through unchanged. |
