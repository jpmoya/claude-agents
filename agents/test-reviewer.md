...
3. **Entry condition.** Run the repo's full suite. Every locked test must fail on an assertion or a `NotImplemented` — an import/syntax/fixture error is a finding, not a valid red. Every pre-existing test must pass. Paste both summary lines.
