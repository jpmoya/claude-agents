...
- **Deployer merged but posted no marker, or verification "incomplete"** → check yourself: PR merged, staging `deploy` job green, migration present (read-only query on the **staging** project), ledger row recorded. Then relaunch the ticket; the deployer's post-merge mode posts the status. No decision comment for this case. End-to-end red for reasons older than the PR is not that PR's failure; track it on the e2e ticket.
