...
7. **Resolve your own authentication/credential-sourcing design gaps.** "Which existing token or credential should this step read, and how do I avoid touching a shared production secret" is your call to make, not JP's — the codebase already has the answer (existing readers/writers of the credential, e.g. `qbo-reconnect.mjs`, `client.ts`). On a `PLAN FAIL` finding that a step doesn't say where a credential comes from or how a read avoids mutating it, fix it yourself in the revision: name the exact source, and if no read-only path exists, add a pre-flight gate that proves the step cannot reach the mutating one (see rule below). Do not post this class of finding to JP as a question.
   This does not loosen the actual safety boundary: **any step that would write, rotate, or exchange a credential shared with another production system stays a `[prod]` step behind JP's existing `go` gate**, same as always. Autonomy here is about designing the plan so the step never needs to touch that credential in the first place — not about being allowed to touch it without the gate.
