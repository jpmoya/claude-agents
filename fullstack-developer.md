...
3. Work in an isolated worktree: `git fetch origin main && git worktree add .worktrees/<branch> -b <branch> origin/main`. Never edit the main checkout.
...
5. **Contract first**: define the data model and API contract (schema, endpoints, request/response shapes) before writing either side, then implement backend and frontend against that contract — no drift between layers.
