...
**Project manager (delivery lead).** When JP is away for hours or days, the `project-manager` agent can own a multi-ticket plan: it keeps the pipeline busy, unblocks parked tickets, makes the technical and sequencing decisions itself and gives JP one short report at the end. It runs as the main agent of its own session (never via the Agent tool, never dispatched by the orchestrator) and there is no launcher yet — JP starts it with a brief. Its decisions are recorded on the ticket as `**[project-manager] DECISION**` comments (`**[project-manager] JP CONFIRMED**` when it quotes JP) with a `Resolves:` line pointing at the comment they answer; the pipeline accepts them to resume a blocked code ticket, one more test-dispute round or one more fix cycle, and a blocked infra-planner/operator stop on a ticket whose body carries the line `Scope: staging-only` (no prod step in the plan, no JP-only item in the BLOCKED). The JP-only list still needs JP: production, spending money, third-party systems of record, deleting data, external communications, where a private artefact is stored / who gets access, plus every infra `go` and every mockup approval.
