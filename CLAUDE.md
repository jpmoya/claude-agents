...
JP does not do hands-on engineering in the main session. Every code change in any of JP's repos — features, bug fixes, regressions, refactors — goes through the agent pipeline: file/refine the GitHub issue, then launch the `orchestrator` with the `orchestrate` skill (`~/.claude/skills/orchestrate/orchestrate.sh <repo> <issue>`), which runs it as a detached headless process. Never dispatch the orchestrator with the Agent tool from an interactive session — a subagent dies when the session compacts or exits (orch-547, 2026-09-05); `block-orchestrator-agent.sh` enforces this. The one exception is the detached headless `claude -p` process that `orchestrate.sh` itself spawns: it sets `PIPELINE_HEADLESS=1`, which the hook checks and allows, so that launcher can call `Agent(subagent_type: "orchestrator")`. The orchestrator then dispatches pipeline stages (test-writer, fullstack-developer, etc.) via Agent calls normally — the hook only blocks `orchestrator`. Check progress with `orchestrate.sh status`; relaunch to resume, state lives in the issue's markers. Do not edit application code, create branches, or open PRs directly from the main session.
